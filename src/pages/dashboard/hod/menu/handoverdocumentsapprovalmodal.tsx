...
interface HandoverDocumentsApprovalModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function HandoverDocumentsApprovalModal({
  isOpen,
  onClose,
  formData,
  setFormData,
  onSubmit,
  isSubmitting,
}: HandoverDocumentsApprovalModalProps) {
  return (
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <FormField label="Handover Notes">
            <div
              ...
              onClick={() =>
                document.getElementById("hod-handover-file-input")?.click()
              }
            >
              <input
                type="file"
                id="hod-handover-file-input"
                className="hidden"
                onChange={(e) => {
                  if (e.target.files?.[0]) {
                    setFormData({
